...
function getNextFriday(date) {
  ...
  const daysUntilFriday = (5 - givenDate.getUTCDay() + 7) % 7 || 7;
  return new Date(
    Date.UTC(
...
function formatDate(date) {
  ...
  const minute = givenDate.getMinutes().toString().padStart(2, 0);
  const second = givenDate.getSeconds().toString().padStart(2, 0);
  ...
  return `${month}/${day}/${year}, ${hour}:${minute}:${second} ${pmAm}`;
}
...
function getWeekNumberByDate(date) {
  ...
  firstMonday.setDate(
    firstThursday.getDate() -
      (firstThursday.getDay() === 0 ? 6 : firstThursday.getDay() - 1)
  );

  const millisecondsDifference = date - firstMonday;
